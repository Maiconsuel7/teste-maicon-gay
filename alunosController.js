...
export async function criarAluno(req, res) {
  ...
  // Certifique-se de que 'prisma' e 'selectSemSenha' estão definidos e importados no seu controller
  try {
    const alunoCriado = await prisma.aluno.create({
      ...
      select: selectSemSenha, // Garante que a senhaHash não seja retornada
    });

    // 3. Retorne res.status(201).json(alunoCriado)
    return res.status(201).json(alunoCriado);
  } catch (error) {
    // Em caso de erro (ex: email duplicado), retorne um status de erro apropriado
    console.error("Erro ao criar aluno:", error);
    return res.status(400).json({ erro: "Não foi possível criar o aluno. Verifique os dados fornecidos." });
  }
}
...
export async function deletarAluno(req, res) {
  ...
  try {
    ...
    return res.status(204).end(); // Retorna 204 sem conteúdo
  } catch (error) {
    // No catch: retorne status 404 com { erro: 'Aluno não encontrado' }
